...
- **Security.** Always escape spec content embedded in `<script>` (`< > → \u003c \u003e`); escape any spec text rendered as HTML via `escapeHtml`. Server stays bound to `127.0.0.1`. Honor parser size limits.
